...
import static com.acme.kafka.connect.speed.WeatherDataSchema.*;

public class SpeedSourceTask extends SourceTask {

    private static Logger log = LoggerFactory.getLogger(SpeedSourceTask.class);

    private SpeedSourceConnectorConfig config;
    private String name;
    ...
    public void start(Map<String, String> properties) {
        config = new SpeedSourceConnectorConfig(properties);
        ...
        Long currentTimetoken = timestampNow * tenThousand;

        log.info("Retrieving data from " + currentTimetoken);

        // set initial offsets to start timetoken
        for (String source : sources) {
            offset.put(source, currentTimetoken);
        }
    ...
    public List<SourceRecord> poll() throws InterruptedException {
        ...
        for (String source : sources) {
            Long currentTimetoken = offset.get(source);
            String topic = "stream";

            // fetch first page and store in records
            final Long timeToken;
            timeToken = currentTimetoken;
                    ...
                    if (result.getChannels().size() != 0) {
                        ...
                        for (List<PNFetchMessageItem> items : result.getChannels().values()) {
                            for (PNFetchMessageItem item : items) {
                                if (item.getTimetoken() > currentTimetoken) {
                                    JsonElement msg = item.getMessage();
                                    itemTimetoken = item.getTimetoken();
                                    log.info("- fetched source " + source + " at timestamp: " + itemTimetoken);
                                    ...
                                    records.add(new SourceRecord(
                                            ...
                                            Collections.singletonMap("offset", currentTimetoken),
